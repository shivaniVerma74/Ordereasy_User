...
class StatePosts extends State<PostsScreen> with TickerProviderStateMixin {
  ...
  List<Widget> _buildDots(int index) {
    List<Widget> dots = [];
    for (int i = 0; i < tempList[index].postImage!.length; i++) {
      dots.add(
        Container(
        ...
        child: Column(
          ...
          children: [
              ...
              child: Row(
                ...
                children: [
                  Text(
                    ...
                    "" : "Expiry Time : ${tempList[i].endDate}", style: TextStyle(
                      color: Colors.red
                  ),
                  ),
                  // Row(
            ...
            Padding(
              padding: const EdgeInsets.only(left: 8.0, right: 8, top: 4, bottom: 4),
              child: Text(
                tempList[i].startDate == "" || tempList[i].startDate == null ?
                "" : "Expiry Date : ${tempList[i].startDate}", style: TextStyle(
                  color: Colors.red
              ),
              ),
            ),
            ...
            Container(
              ...
              child: Container(
                child:
                tempList[i].postImage!.isEmpty || tempList[i].postImage![0].toString() == imageUrl
                    ? Image.asset(
                  'assets/logo/mt_logo.png',
                  // widget.snap['postUrl'],
                  fit: BoxFit.fill,
                ...
                children: [
                  Row(
                    children: [
                      ...
                      Text(tempList[i].startDate.toString(), style: TextStyle(
                          color: Theme.of(context).colorScheme.fontColor
                      ),),
                    ],
                  ),
  ...
  removeFromCart(int index) async {
    ...
    if (_isNetworkAvail) {
      if (CUR_USERID != null) {
        ...
        print("params________post1___ ${parameter}");
...
/*  getAppbar() {
  ...
  Widget productItem(int index, bool pad) {
    if (index < productList.length) {
      ...
      if (model.prVarientList![model.selVarient!].disPrice! != "0") {
        off = (double.parse(model.prVarientList![model.selVarient!].price!) -
            double.parse(model.prVarientList![model.selVarient!].disPrice!)).toDouble();
  ...
  Future<void> addToCart(int index, String qty) async {
    ...
    if (_isNetworkAvail) {
      if (CUR_USERID != null) {
        ...
        print("params________post22___ ${parameter}");
