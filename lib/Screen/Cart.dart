...
class StateCart extends State<Cart> with TickerProviderStateMixin {
  ...
  Widget listItem(int index, List<SectionModel> cartList) {
    ...
    if (cartList[index].productList![0].prVarientList != null) {
      for (int i = 0;
      ...
      price = double.parse(cartList[index]
          .productList![0]
      ...
      print(
          "cartList**avail****${cartList[index].productList![0].availability}");

      if (_controller.length < index + 1) {
        _controller.add(new TextEditingController());
      }
      if (cartList[index].productList![0].availability != "0" ||
          cartList[index].qty != null) {
  ...
  String? deliveryType;
  String? order_delivery_onoff;
  String? new_seller_id;
    ...
    if (_isNetworkAvail) {
      try {
        ...
        if (!error) {
          ...
          new_seller_id = data[0]['product_details'][0]['seller_id'];
          log("_getCart_api_response_________ $data and_new_seller_id____________  $new_seller_id");

          oriPrice = double.parse(getdata[SUB_TOTAL]);

          taxPer = double.parse(getdata[TAX_PER]);
          deliveryType = data[0]['seller_oders_type'];
          order_delivery_onoff = data[0]['order_delivery'].toString();
  ...
  Future<void> addToCart(
    ...
    if (_isNetworkAvail) {
      try {
        ...
        print("params________1___ ${parameter}");
        Response response =
  ...
  Future<void> addToCartCheckout(
    ...
    if (_isNetworkAvail) {
      try {
        ...
        print("params________2___ ${parameter}");
        Response response =
            await post(manageCartApi, body: parameter, headers: headers)
                .timeout(Duration(seconds: timeOut));
  ...
  saveForLater(String? id, String save, String? qty, double price,
    ...
    if (_isNetworkAvail) {
      try {
        ...
        print("params________3___ ${parameter}");
    ...
    if (!remove &&
      ...
      if (_isNetworkAvail) {
        try {
          ...
          print("params________4___ ${parameter}");
          ...
          if (response.statusCode == 200) {
            ...
            print("params________4___ ${manageCartApi}");
            bool error = getdata["error"];
            String? msg = getdata["message"];
            if (!error) {
              var data = getdata["data"];
    ...
    if (!remove &&
      ...
      if (_isNetworkAvail) {
        try {
          ...
          print("params________5___ ${parameter}");
          Response response =
              await post(manageCartApi, body: parameter, headers: headers)
                  .timeout(Duration(seconds: timeOut));
          ...
          if (!error) {
            ...
            if (move == false) {
              ...
              if (data[SUB_TOTAL] != null || data[SUB_TOTAL] != '') {
                oriPrice = double.parse(data[SUB_TOTAL].toString() ?? '0');
              } else {
                oriPrice = 0;
              }
  ...
  getSharedData() async{
    ...
    print("user od is $User_id");
    print("user mobile is cart pagee$User_mobile");
  }
    ...
    if (_isNetworkAvail) {
      ...
      try {
        var parameter = {
          ...
          ORDER_TYPE: deliveryType == 'take_away'
              ? "Take Away"
              : deliveryTypeIndex == 1
                  ? "Take Away"
                  : "Delivery"
        ...
        print("deliveryType is $deliveryType");
        ...
        if (response.statusCode == 200) {
          var getdata = json.decode(response.body);
          print(placeOrderApi);
          print("place order response=============${getdata.toString()}");
          bool error = getdata["error"];
          String? msg = getdata["message"];
          if (!error) {
  ...
  payment(List<SectionModel> cartList) {
    return Card(
      elevation: 0,
      child: InkWell(
        ...
        onTap: () async {
          ...
          await Navigator.push(
              ...
              MaterialPageRoute(
                  builder: (BuildContext context) =>
                      Payment(updateCheckout, msg, new_seller_id)));
          if (mounted) checkoutState!(() {});
        },
        child: Padding(
          padding: const EdgeInsets.all(8.0),
                                ...
                                children: [
                                  ...
                                  deliveryType == "take_away"
                                      ? Container(
                                          ...
                                          child: Center(
                                              child: Text(
                                            getTranslated(context, "TAKEAWAY")!,
                                            style: Theme.of(context)
                                                .textTheme
                                                .subtitle2!
                                                .copyWith(
