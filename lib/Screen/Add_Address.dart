...
class StateAddress extends State<AddAddress> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (widget.update!) {
      ...
      checkedDefault = item.isDefault == "0" ? true : false;
    } else {
      getCurrentLoc();
    }
  }
                ...
                child: TextFormField(
                  ...
                  decoration: InputDecoration(
                    ...
                    suffixIcon: IconButton(
                      ...
                      //   if (addressC?.text != "") {
                      //     try {
                      //       final coordinates = await locationFromAddress(addressC!.text);
                      //       setState(() {
                      //         _coordinates = coordinates.toString() ;
                      //       });
                      //
                      //       List listo = _coordinates.split("");
                      //       print("listo ${listo}");
                      //      // print('Latitude: is=${loc.replaceAll(" ", "").substring(11,20)}');
                      //     } catch (e) {
                      //       print(e);
                      //     }
                      //   }
                      ...
                      onPressed: () async {
                       ...
                       Position position = await Geolocator.getCurrentPosition(desiredAccuracy: LocationAccuracy.high);
                          ...
                          MaterialPageRoute(
                            builder: (context) => PlacePicker(
                              ...
                              onPlacePicked: (result) {
                                print("aaaaaaaaaaaaaaaaaaaaaa  ${result.formattedAddress}");
                                setState(() {
                                  addressC?.text = result.formattedAddress.toString();
                                  pickLat = result.geometry!.location.lat;
                                  pickLong = result.geometry!.location.lng;
                                });
                                print("address_after_select____________${addressC?.text}");
                                Navigator.of(context).pop();
                              },

                              initialPosition: LatLng(latitude == null || latitude == "" ? pickLat: double.parse(pickLat.toString()),
                                longitude == null || longitude == "" ? pickLong: double.parse(pickLong.toString()),),
                        ...
                        print("address_after_select____________${addressC?.text}");
                        print("lat is ${pickLat} long is ${ pickLong}");
  ...
  Future<void> addNewAddress() async {
    ...
    try {
      ...
      print("checking_saved_data ${data}");
        ...
        if (!error) {
          ...
          if (widget.update!) {
            if (checkedDefault.toString() == "true" ||
              ...
              if (!ISFLAT_DEL|| addressList[selectedAddress!].freeAmt != null || addressList[selectedAddress!].freeAmt != "") {
                if (oriPrice < double.parse((addressList[selectedAddress!].freeAmt ?? '0.0') == '' ? '0.0' : addressList[selectedAddress!].freeAmt! )) {
                  delCharge = double.parse((addressList[selectedAddress!].deliveryCharge ?? '0.0') == '' ? '0.0' : addressList[selectedAddress!].deliveryCharge!);
                } else {
                  delCharge = 0;
                }
              ...
              if (!ISFLAT_DEL) {
                if (oriPrice <
                    double.parse((addressList[selectedAddress!].freeAmt ?? '0.0') == '' ? '0.0' : addressList[selectedAddress!].freeAmt!)) {
                  delCharge = double.parse(
                      (addressList[selectedAddress!].deliveryCharge ?? '0.0') == '' ? '0.0' : addressList[selectedAddress!].deliveryCharge!);
                } else {
                  delCharge = 0;
                }
                totalPrice = totalPrice + delCharge;
            ...
            if (checkedDefault.toString() == "true" ||
              ...
              if (!ISFLAT_DEL && addressList.length != 1 || addressList[selectedAddress!].freeAmt != null || addressList[selectedAddress!].freeAmt != "") {
