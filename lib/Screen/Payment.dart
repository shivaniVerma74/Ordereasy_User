...
class Payment extends StatefulWidget {
  ...
  Payment(this.update, this.msg, this.seller_id);
...
class StatePayment extends State<Payment> with TickerProviderStateMixin {
            ...
            for (int i = 0; i < timeSlotList.length; i++) {
              ...
              if (cur.isBefore(last)) {
                timeModel.add(new RadioModel(
                    ...
                    name: "${timeSlotList[i].name}  ${formatTime(timeSlotList[i].fromTime.toString(), DateFormat('HH:mm'))} - ${formatTime(timeSlotList[i].toTime.toString(), DateFormat('HH:mm'))}",
                    img: ''));
              }
            }
          }
  ...
  Future<void> _getdateTime() async {
    print("seller_id_is_____________${widget.seller_id}");
    _isNetworkAvail = await isNetworkAvailable();
    if (_isNetworkAvail) {
      timeSlotList.clear();
      try {
        ...
        if (response.statusCode == 200) {
          ...
          log("printing response==> ${getdata["data"]}");
          ...
          if (!error) {
            ...
            print("time_slotsApi_response_is__ $jsonResponse");
            var timeSlots = jsonResponse["data"];
            ///--------------------------
            // var timeSlots = data["time_slots"];
            timeSlotList = (timeSlots as List).map((timeSlots) => new Model.fromTimeSlot(timeSlots)).toList();
            if (timeSlotList.length > 0) {
              for (int i = 0; i < timeSlotList.length; i++) {
                if (selectedDate != null) {
                  ...
                  if (date == tdDate) {
                    DateTime cur = DateTime.now();
                    String time = timeSlotList[i].lastTime!;
                    // log("priniting response of payment method screen== ${timeSlotList}");
                    DateTime last = DateTime(
                        cur.year,
                        cur.month,
                        cur.day,
