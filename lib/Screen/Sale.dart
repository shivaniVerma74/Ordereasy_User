...
class _SaleState extends State<Sale>
  ...
  @override
  void initState() {
    print('sale screen initstate called');
    //getSlider();
    // getCat();
    getRecentSearch();
    getProduct("0");
  ...
  callChat(index) async {
    ...
    print("fuid is  " + recentSearchModel!.date![index].fuid.toString());
    var otherUser1 = types.User(
      firstName: recentSearchModel!.date![index].username.toString(),
      id: recentSearchModel!.date![index].id.toString().trim(),
      imageUrl: 'https://i.pravatar.cc/300?u=${recentSearchModel!.date![index].email}',
      lastName: "",
    );

    print("otherUser1 is == $otherUser1  ");

    _handlePressed(otherUser1, context, recentSearchModel!.date![index].fuid.toString(), recentSearchModel!.date![index]);
  }

  _handlePressed( types.User otherUser, BuildContext context, String fcmID, Date2 model) async {
    print("fcmID_is__________ ${fcmID} User_email $User_email");
    if(fcmID == null || fcmID == ""){
      print("fuid is null");
      setState(() {
        chatLoading = false;
     ...
     // setSnackbar(getTranslated(context, 'availMsg')!, context);
      ...
      print("room is ${room.runtimeType}");
      setState(() {
        chatLoading = false;
        iconTapped = false;
      });
  ...
  _bakeries() {
    return recentSearchModel == null ? Container(height:500, child: Center(child: CircularProgressIndicator(color: colors.primary,),)) : recentSearchModel!.date == null ? SizedBox() : Padding(
              ...
              itemBuilder: (context, index) {
                return Padding(
                  ...
                  child: GestureDetector(
                    onTap: () async{
                      selected_seller_id = recentSearchModel!.date![index].id.toString();
                      // Navigator.push(
                      //     context,
                      //     MaterialPageRoute(
                      ...
                      print("data before sending ${recentSearchModel!.date![index].id.toString()}");

                      if(recentSearchModel?.date?[index].active_status.toString() == "1") {
                       await Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (context) => RestaurantDashboard(
                                  title: recentSearchModel!.date![index].username.toString(),
                                  sellerId: recentSearchModel!.date![index].id.toString(),
                                  sellerData: jsonEncode(recentSearchModel!.date![index]),
                                  subCatName: jsonEncode(recentSearchModel!.date![index].username),
                                  subCatAddress: jsonEncode(recentSearchModel!.date![index].address),
                                  subCatLogo: recentSearchModel!.date![index].logo,
                                  seller_email: jsonEncode(recentSearchModel!.date![index].email),
                                  seller_fuid: jsonEncode(recentSearchModel!.date![index].fuid),
                                  seller_mob: jsonEncode(recentSearchModel!.date![index].mobile),
                                  showAll: true,
                                  // catId: ,//0000000000000
                                )));
                       getRecentSearch();
                      }else{
                        setSnackbar(getTranslated(context, 'disableMsg')!, context);
                      }
                      ...
                      children: <Widget>[
                        Card(
                          ...
                          child: Container(
                            ...
                            child: Row(
                              children: [
                                if(recentSearchModel?.date?[index].logo != null)
                                Container(
                                  height: 60,
                                  width: 60,
                                  decoration: BoxDecoration(
                                ...
                                Expanded(
                                  child: Column(
                                    children: [
                                      if(recentSearchModel?.date?[index].active_status.toString() == "0")
                                      Padding(
                                        padding: const EdgeInsets.only(top: 5, right: 10),
                                        child: Align(
                                            alignment: Alignment.centerRight,
                                            child: Text("${getTranslated(context, 'Disable')}",style: TextStyle(color: Colors.grey,fontSize: 15,fontWeight: FontWeight.w600),)),
                                      ),
                                      ListTile(
                                          dense: true,
                                          title: Text(
                                                  ...
                                                  onTap:() async{
                                                    await showDialog(
                                                        ...
                                                        builder: (_) => AlertDialog(
                                                          title: Text(
                                                            getTranslated(context, 'deleteTXT')!,
                                      ...
                                      Padding(
                                        ...
                                        child: Row(
                                          ...
                                          children: [
                                            ...
                                            //
                                            //     // if(iconTapped!=true)
                                            //     // print('fcm id is ${recentSearchModel!.date![index].fuid.toString()}');
                                            //     iconTapped == true? null : callChat(index);
                                            //     if(iconTapped==true){
                                            //       setSnackbar(getTranslated(context, 'chatMsg')!, context);
                                            //     }
  ...
  getRecentSearch()async{
    ...
    print('url is ${Uri.parse('${baseUrl}recent_search')}');
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      var finalResult = await response.stream.bytesToString();
      ...
      log("restaurants list == ${jsonEncode(recentSearchModel!.date)}");
    }
    else {
      print(response.reasonPhrase);
    }
